...
import { ContainerPost, Content, MenuBody, PostCard, Published, Title } from "./Style";
    ...
    <MenuBody>
      ...
      {post.map((post: any) => (
        <PostCard key={post.id}>
          ...
          <div>
            <span>Category: {post.categories?.map((category: any) => (
              <span className="category-item" key={category.id}>{category.name}</span>
            ))}</span>
          </div>
        </PostCard>
      ))}
